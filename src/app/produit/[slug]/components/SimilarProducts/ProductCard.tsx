...
export const ProductCard: React.FC<ProductCardProps> = memo(({ product, index, itemVariants, onProductClick }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-700 overflow-hidden 
                 min-h-auto sm:min-h-[520px] lg:min-h-[540px] xl:min-h-[560px] 
    ...
    >
      ...
      <ProductImage product={product} discountPercentage={discountPercentage} index={index} />

      {/* Product Content Section */}
      <ProductContent product={product} savings={savings} />
    </motion.div>
  )
})
