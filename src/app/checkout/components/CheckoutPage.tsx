...
import { useCartItems } from '@/app/panier/store/cart';
import { CheckoutProvider } from '../context/CheckoutContext';
import CustomerInfoStep from './steps/CustomerInfoStep';
import OrderSummaryStep from './steps/OrderSummaryStep';
import SuccessStep from './steps/SuccessStep';

// This is the core UI component for the checkout page.
// It's responsible for laying out the steps and navigation.
// The actual logic is handled by the useCheckoutForm hook.
const CheckoutFlow: React.FC = () => {
  ...
  // Redirect to cart if it's empty and we are not on the success step
  useEffect(() => {
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 2:
        return <OrderSummaryStep form={form} onSubmit={onSubmit} isSubmitting={isSubmitting} />;
      case 3:
        return <SuccessStep />;
      default:
        return <CustomerInfoStep form={form} />;
    ...
    <div className="min-h-screen bg-gradient-to-br from-brand-darkGreen-50 via-white to-brand-sage-50 py-12">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {currentStep < 3 && (
          ...
          >
            <Button onClick={prevStep} disabled={currentStep === 1} variant="outline" size="lg" icon={<ArrowLeft className="w-5 h-5" />}>
              Retour
            </Button>
            {currentStep === 1 && (
              <Button onClick={nextStep} size="lg" icon={<ArrowRight className="w-5 h-5" />} iconPosition="right">
                Continuer
              </Button>
            )}
            {currentStep === 2 && (
              <Button onClick={onSubmit} disabled={isSubmitting} size="lg" icon={<ArrowRight className="w-5 h-5" />} iconPosition="right">
...
// The main export wraps the UI in the provider, so any child component
// can access the checkout context if needed.
