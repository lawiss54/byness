...
const CustomerInfoStep: React.FC<CustomerInfoStepProps> = ({ form }) => {
  ...
  const { setShippingCost } = useCartCheckout();
  ...
  }, [shippingInfo, selectedMunicipality, shippingType, setShippingCost, setValue]);
    ...
    <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="space-y-8">
      ...
      <div className="mt-8">
        <RadioGroup
          ...
          value={shippingType}
          onChange={(val) => setValue('shippingType', val as 'home' | 'desk')}
          name="shippingType"
